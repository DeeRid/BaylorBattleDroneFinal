  ...
  */
  ...
  uint8_t propellerMode = 0;     // Propellor Mode
  ...
  int startThrottle = 1050;    //Starting place for Throttle
  int defStartThrottle = startThrottle;  // To set it back later
  int incThrottle = 1;        //How much to increment throttle
  int accToReach = -500;       // Lower or Negative Means faster
  boolean accReached = false; // Has the acceleration been reached
  unsigned long timeToLaunch = 200000;  //time in microseconds
  unsigned long timeToStable = 500000;   // time in microseconds
  unsigned long deathTime = 0;
  int launchThrot = 1800;          // Throttle during launch best 1350,-50
  int stableThrot = launchThrot - 5;          // Throttle to stabalize before altitude is maintained
  ...
  int count = 0;
  int flightDegSign = 1;
  static uint32_t firingTime = 0;

  
  
  //uint8_t hits = 0; // counter for the number of hits recieved by a drone
  //unsigned long timeInBetweenHits = 500000;
  #define IR_TRANS              A4                 
  ...
  void setup()
  {
      /*********************** Baylor University Setup Information ***********************/
  ...
  void go_disarm() {
    ...
    /*************************** Baylor University Disarm Code ***************************/
    isLaunch = false;
    launchCalledHover = false;
    accReached = false;
  
  /************************* Baylor University Disarm Code End *************************/
  ...
  void loop () {
      ...
      /***********************************************************************************/
    /******************************** Baylor Main Loop Code ****************************/
    /***********************************************************************************/
  
    /* The following code was written for hit detection and to handle firing. */
  
  
     if(isFiring && (millis() - firingTime) >= interval){
      isFiring = false;
     }
    ...
    if (f.ARMED && isLaunch) {
      ...
      //if (!accReached) {
        if (tstart == 0 && rcData[THROTTLE] < MAXTHROTTLE) {
        ...
        if(tstart > 0 && micros() - tstart > 150000){
          //rcData[THROTTLE] = (rcData[THROTTLE] - 1000)*.95 + 1000;
          isLaunch = false;
          f.BARO_MODE = 1;
          AltHold = EstAlt;
...
//    // Baylor dying code
  ...
  switch(n){
    ...
    case 3: // Two Hit
    {
        if(count > 0 ||(micros() - lastMove) > 4000000 ){
          if(flightDegSign > 0){
            rcData[THROTTLE] = 2000;
            count++;
          }
  ...
  if(deathTime != 0 && micros() - deathTime > 4000000){
    rcData[THROTTLE] = 0;
    go_disarm();
  }
